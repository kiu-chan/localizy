...
import '../pages/assignment_map_page.dart';
import 'validation_badges.dart';

/// Bottom sheet chi tiết một yêu cầu xác minh, kèm các hành động
/// confirm appointment / verify / reject theo trạng thái.
///
/// Pop với `true` khi có hành động thành công để caller reload danh sách.
class AssignmentDetailSheet extends ConsumerStatefulWidget {
  final ValidationAssignment assignment;

  const AssignmentDetailSheet({super.key, required this.assignment});

  @override
  ConsumerState<AssignmentDetailSheet> createState() =>
      _AssignmentDetailSheetState();
}

class _AssignmentDetailSheetState extends ConsumerState<AssignmentDetailSheet> {
  late ValidationAssignment _a;
  bool _actionLoading = false;

  ValidatorRepository get _repo => ref.read(validatorRepositoryProvider);
          ...
          children: [
            ...
            Row(
              children: [
                ...
                ValidationStatusBadge(status: _a.status),
              ],
            ),
            const SizedBox(height: 20),
            ...
            _row(Icons.calendar_today, l10n.validatorSubmitted,
                _a.submittedDate != null
                    ? formatValidationDateTime(_a.submittedDate!)
                    : '-'),
            _row(Icons.assignment_ind, l10n.validatorAssignedDate,
                _a.assignedDate != null
                    ? formatValidationDateTime(_a.assignedDate!)
                    : '-'),
            ...
            if (_a.address != null) ...[
              ...
              _row(Icons.location_city, l10n.validatorCityCode,
                  _a.address!.cityCode),
            ...
            if (_a.verificationData != null) ...[
              ...
              _check(l10n.validatorPhotosProvided,
                  _a.verificationData!.photosProvided),
              _check(l10n.validatorDocumentsProvided,
                  _a.verificationData!.documentsProvided),
              _check(l10n.validatorLocationVerified,
            ...
            if (_a.assignedValidator != null) ...[
              ...
              _row(Icons.badge, l10n.validatorNameLabel,
                  _a.assignedValidator!.name),
            ],
            ...
            if (_a.processedBy != null ||
              ...
              if (_a.processedDate != null)
                _row(Icons.check_circle_outline, l10n.validatorProcessedDate,
                    formatValidationDateTime(_a.processedDate!)),
              if (_a.processingNotes != null && _a.processingNotes!.isNotEmpty)
                _row(Icons.comment, l10n.validatorProcessingNotes,
                    _a.processingNotes!),
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    if (_a.address != null)
                      _dialogInfoRow(Icons.location_on, _a.address!.code),
  ...
  Future<void> _doConfirmAppointment() async {
    ...
    try {
      await _repo.confirmAppointment(_a.id);
      setState(() => _actionLoading = false);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                AppLocalizations.of(context)!.validatorAppointmentConfirmed),
                      ...
                      onPressed: () {
                        ...
                        if (reason.isEmpty) {
                          ScaffoldMessenger.of(ctx).showSnackBar(
                            SnackBar(
                                content: Text(l10n.validatorReasonRequired)),
                          );
                          return;
                        }
                        Navigator.pop(ctx);
  ...
  Future<void> _doVerify(String notes) async {
    setState(() => _actionLoading = true);
    try {
      await _repo.verifyValidation(_a.id, notes);
      setState(() => _actionLoading = false);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text(AppLocalizations.of(context)!.validatorAddressVerified),
            backgroundColor: Colors.green,
          ),
        );
        Navigator.pop(context, true);
  ...
  Future<void> _doReject(String reason) async {
    setState(() => _actionLoading = true);
    try {
      await _repo.rejectValidation(_a.id, reason);
      setState(() => _actionLoading = false);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text(AppLocalizations.of(context)!.validatorAddressRejected),
  ...
  // ── UI helpers ───────────────────────────────────────────────────────────────
  Widget _buildAttachmentThumbnails(
      ValidationVerificationData data, AppLocalizations l10n) {
