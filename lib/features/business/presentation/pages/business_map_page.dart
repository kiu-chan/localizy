...
import '../widgets/location_list_sheet.dart';
import '../widgets/map_form_snapshot.dart';
import '../widgets/map_normal_overlay.dart';
import '../widgets/picking_mode_overlay.dart';

class BusinessMapPage extends ConsumerStatefulWidget {
  const BusinessMapPage({super.key});

  @override
  ConsumerState<BusinessMapPage> createState() => _BusinessMapPageState();
}

class _BusinessMapPageState extends ConsumerState<BusinessMapPage> {
  ...
  Future<void> _loadAddresses() async {
    ...
    try {
      ...
      final list = _showMineOnly
          ? await repo.getBusinessMineAddresses()
          : await repo.getBusinessAddresses();
      if (mounted) {
  ...
  Future<void> _submitNewAddress(AddLocationSubmitResult data) async {
    ...
    try {
      final newAddress = await ref.read(addressRepositoryProvider).addAddress(
        name: data.name,
