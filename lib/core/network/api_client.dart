...
/// Provider chuẩn để lấy ApiClient trong code mới (Riverpod).
///
/// ```dart
/// final client = ref.read(apiClientProvider);
/// final data = await client.getJson('api/...');
/// ```
final apiClientProvider = Provider<ApiClient>((ref) => ApiClient.fromEnv());

/// HTTP client dùng chung của app (trước đây là MainApi).
///
/// Tự động đính kèm Authorization header nếu có token trong secure storage.
class ApiClient {
  ApiClient({required this.baseUrl, FlutterSecureStorage? storage})
      : _secureStorage = storage ?? const FlutterSecureStorage();

  /// Tạo client với API_BASE_URL đọc từ .env.
  /// dotenv.load(...) phải được gọi trước (trong ConfigManager.initialize).
  ...
  Future<http.Response> get(String pathOrUrl,
  ...
  /// Send DELETE request.
  Future<http.Response> delete(String pathOrUrl,
  ...
  Future<http.Response> postJson(String pathOrUrl, Object body,
      {Map<String, String>? headers}) async {
    final uri = _buildUri(pathOrUrl);
  ...
  Future<http.Response> putJson(String pathOrUrl, Object body,
      {Map<String, String>? headers}) async {
    final uri = _buildUri(pathOrUrl);
    final h = await _mergeHeaders(headers, 'application/json');
    return _checkUnauthorized(
  ...
  Future<http.Response> patchJson(String pathOrUrl, Object body,
      {Map<String, String>? headers}) async {
    final uri = _buildUri(pathOrUrl);
    final h = await _mergeHeaders(headers, 'application/json');
    return _checkUnauthorized(
  ...
  /// code is not 2xx.
  Future<dynamic> getJson(String pathOrUrl,
      {Map<String, String>? headers}) async {
    final resp = await get(pathOrUrl, headers: headers);
    if (resp.statusCode >= 200 && resp.statusCode < 300) {
      return json.decode(resp.body);
    }
    throw ApiException.fromResponse('GET', pathOrUrl, resp);
  }
}
